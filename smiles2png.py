...
def generate_aligned_coordinates(mols: List[Chem.rdchem.Mol], align: List):
  ...
  """
    ...
    if no_match > 0:
        print(
            f"Warning {no_match} of {len(mols)} did not match the alignment template",
            file=sys.stderr)


def generate_plots_grid(mols: List[Chem.rdchem.Mol], png_stem: str,
...
def display_pngs(pngs: List[str], config: Smiles2PngConfig):
...
def read_smiles(input_stream, lines: List[str],
...
def get_smiles(smiles_fname: str, lines: List[str],
...
def do_smiles2png(smiles_fname: List[str],
                  smiles_on_command_line: Optional[List[str]], png_stem: str,
...
def smiles2png(argv):
    ...
    if png_stem:
        ...
        png_stem = generate_png_stem(smiles_fname, smiles_on_command_line,
                                     config)
